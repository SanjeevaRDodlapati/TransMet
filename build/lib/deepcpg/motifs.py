...
def read_tomtom(path):
    ...
    d['idx'] = [int(x) for x in d['query_id'].str.replace('filter', '')]
    return d
...
def get_report(filter_stats_file, tomtom_file, meme_motifs):
    ...
    tomtom = tomtom.loc[:, ~tomtom.columns.isin(['query_id', 'optimal_offset'])]
    d = pd.merge(filter_stats, tomtom, on='idx', how='outer')
    meme_motifs = meme_motifs.rename(columns={'id': 'target_id'})
    d = pd.merge(d, meme_motifs, on='target_id', how='left')
    d.index.name = None
    return d
