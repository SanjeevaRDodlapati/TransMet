...
def extract_seq_windows(seq, pos, wlen, seq_index=1, assert_cpg=False):
    ...
    enc_seq = int_to_onehot(seq_wins, dim=4)
    return enc_seq
...
#         make_dir(opts.out_dir)
        ...
        for chromo in pos_table.chromo.unique():
            ...
            if 'cpg_mat' in chromo_outputs and opts.cpg_cov:
                ...
                idx = cov >= opts.cpg_cov # Drops pos with lower coverage
            ...
            for chunk in range(nb_chunk):
                ...
                # Write positions
                chunk_file.create_dataset('chrom', shape=(len(chunk_pos),),
                                          dtype='S2')
                chunk_file['chrom'][:] = chromo.encode()
                chunk_file.create_dataset('coor', data=chunk_pos, dtype=np.int32)
                chunk_file.create_dataset('features',
                                data=[name for name, value in six.iteritems(chunk_outputs['cpg'])],
                                          compression='gzip')
                ...
                if chromo_dna:
                    ...
                    dna_wins = extract_seq_windows(chromo_dna, pos=chunk_pos,
                                                   seq_index=opts.seq_index, wlen=opts.dna_wlen)
                    assert len(dna_wins) == len(chunk_pos)
                    chunk_file.create_dataset('sequence', data=np.packbits(dna_wins > 0, axis=1), dtype=np.int8,
                                            compression='gzip')
                    chunk_file.create_dataset('sequence_length', data=opts.dna_wlen, dtype='uint16')

                chunk_file.create_dataset('targets',
                                          data=np.transpose(np.array([value for name, value in six.iteritems(chunk_outputs['cpg'])], dtype=np.float32)),
                                          compression='gzip')
