...
class App(object):
    ...
    def main(self, name, opts):
        ...
        for seed in seeds:
            ...
            log.info('Computing activations - time: "%s"' % str(seed+1))
            ...
            acts_mean = np.mean(acts, axis = (0,1))
            acts_norm = np.subtract(acts, acts_mean)
        ...
        for data in data_reader2:
            ...
            fun_eval = fun(inputs)
            act = fun_eval[0]
            seqs = dna.onehot_to_int(inputs[0])
            # seqs = inputs[0]
        ...
        if opts.motif_dbs:
            ...
            cmd = 'srun /cm/shared/applications/singularity/3.8.4/bin/singularity run /cm/shared/containers/meme/5.5.0/crun.meme tomtom -dist pearson -thresh {thr} -oc {out_dir} ' + \
                  '{meme_file} {motif_dbs}'
