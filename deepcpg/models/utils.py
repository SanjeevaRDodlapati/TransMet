...
def get_conv_layer_act(model, layer_name, get_act=False):
    """Return the first convolutional layers in a stack of layer.
...
def search_model_files(dirname):
    ...
    if pt.isfile(json_file):
        order = ['model_weights.h5', 'model_weights_val.keras',
                 'model_weights_train.keras']
        for name in order:
            filename = pt.join(dirname, name)
            if pt.isfile(filename):
                return [json_file, filename]
    elif pt.isfile(pt.join(dirname, 'model.keras')):
        return pt.join(dirname, 'model.keras')
    else:
        return None
...
def load_model(model_files, custom_objects=CUSTOM_OBJECTS, log=None):
    ...
    if pt.splitext(model_files[0])[1] == '.keras':
...
def get_objectives(output_names, loss_fn, alpha=None, gamma=None):
    ...
    for output_name in output_names:
        _output_name = output_name.split(OUTPUT_SEP)[-1].split('_')[0]
...
def add_output_layers(stem, output_names, loss_fn=None, init='glorot_uniform'):
    ...
    for output_name in output_names:
        ...
        if _output_name[-1] in ['entropy']:
            ...
            # name = output_name.split(OUTPUT_SEP)[-1].split('_')[0] ## Added on 5/22/24
