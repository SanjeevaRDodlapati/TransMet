...
class App(object):
    ...
    def create_parser(self, name):
        ...
        g.add_argument(
            '--collect_grads',
            help='Whether to collect/print gradients or not',
            type=bool,
            default=True)
        g.add_argument(
            '--early_stopping',
            ...
            def train_step(inputs, labels):
                with tf.GradientTape(persistent=True) as tape:
                    ...
                    for i, key in enumerate(list(labels.keys())):
                        loss = loss_fn(labels[key], predictions[i])
                        losses.append(loss)

                    ave_loss = tf.reduce_mean(losses)
                # Compute gradients
                gradients = tape.gradient(ave_loss, model.trainable_variables)
                # Apply gradients
                optimizer.apply_gradients(zip(gradients, model.trainable_variables))

                return ave_loss, gradients
            for epoch in range(opts.nb_epoch):
                for inputs, outputs, weights in train_data:
                    loss, gradients = train_step(inputs, outputs)
                    print(f'Epoch: {epoch}, Loss: {loss}')
                    print(f'Gradients for the first layer: {np.mean(gradients[0].numpy())}')
